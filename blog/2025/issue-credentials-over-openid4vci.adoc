...
OpenID4VCI supports multiple credential formats, which define how verifiable credentials are encoded and exchanged. *Keycloak currently supports SD-JWT VC for issuance*, while other formats are part of the OpenID4VCI specification.

- https://drafts.oauth.net/oauth-sd-jwt-vc/draft-ietf-oauth-sd-jwt-vc.html[SD-JWT VC] – Selective Disclosure JSON Web Token Verifiable Credential
...
These formats allow issuers to provide portable, verifiable digital proofs, supporting selective disclosure and offline verification in decentralized identity ecosystems.

=== Concrete Use-Cases Enabled by OpenID4VCI

Verifiable credentials open up a variety of real-world applications. Examples include:
...
image::${blogImages}/openid4vci/authorization-code-flow.png[alt="Authorization Code Flow Diagram", align=center, width=800]
...
.Pre-Authorized Code Flow Diagram
image::${blogImages}/openid4vci/pre-authorized-code-flow.png[alt="Pre-Authorized Code Flow Diagram", align=center, width=800]

Now that we’ve covered the technical flows, the rest of this guide walks through configuring Keycloak to issue verifiable credentials using the *Pre-Authorized Code Flow*, including realm, client scope, and client-level setup.
