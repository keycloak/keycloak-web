...
The FIPS support in Keycloak means that Keycloak server is able to run on the FIPS compliant OS with FIPS compliant Java. It also means that Keycloak server itself is FIPS compliant and can be used
by parties, which strictly require FIPS 140-2 support. Even if you do not use FIPS enabled OS, you can still try FIPS enabled Keycloak server by using custom `java.security` file with only
