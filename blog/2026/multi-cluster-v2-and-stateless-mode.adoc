...
Read on to learn more, and https://www.youtube.com/watch?v=_htgZ1bSinA[ask questions in our upcoming live Q&A event on Jul 21!]
...
No Infinispan traffic happens between nodes during OIDC or SAML request processing, eliminating transient failures if a node fails unexpectedly. JGroups is still used for intra-cluster realm cache coordination, but it is not touched during authentication flows.
...
* *Automatic failover*: A load balancer can detect site failures and redirect traffic to the surviving site. If database failover is required, a short downtime might occur until the new writer instance becomes available.
...
All user and realm data is replicated synchronously, so persistent data is not lost during failover. Session data is replicated asynchronously by default if the database supports it to reduce request latency.
...
If you are running a multi-cluster v1 deployment, migrating to v2 is straightforward: enable the `stateless` feature flag, then remove the external Infinispan cluster, its configuration, the AWS Lambda, and the Prometheus monitoring and alerts that triggered the AWS lambda.

As all user sessions are stored in the database already in multi-cluster v1 mode, users will still be logged in after the migration to multi-cluster v2.
Information about in-progress logins and login failure counters is lost during the upgrade.

Single-cluster deployments can be upgraded to stateless mode by enabling the `stateless` feature flag and adding the cluster name parameter in a similar way. If you have persistent user sessions enabled (which is the default for single clusters), users will still be logged in when migrating to multi-cluster v2.
