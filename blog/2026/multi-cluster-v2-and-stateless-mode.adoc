...
* *Trade-off:* Approximately 8-10 milliseconds of additional latency per authentication interaction, negligible for interactive login flows. An increase in database CPU load and IOPS by the factor of two.
...
* *Prerequisite:* A synchronously replicated database and a low-latency network between sites.
...
* *Planned outages*: The setup is designed for environments that can tolerate planned maintenance windows for minor and major upgrades of Keycloak.
...
Information for in-progress logins and login failure counters is lost during the upgrade.
