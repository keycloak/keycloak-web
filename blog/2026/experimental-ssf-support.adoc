...
When you revoke a user's session in Keycloak, how long until the SaaS app they're logged into actually signs them out?
Usually: until their next token refresh, which can be minutes, hours, or in some cases never. The same gap exists when
an account is disabled, a credential is rotated, or a device is flagged as non-compliant. Keycloak knows; the relying
parties don't, until they happen to ask again.

The new *Shared Signals Framework* (SSF) support, shipping experimentally in Keycloak, closes that gap. Keycloak can
now push signed Security Event Tokens (SETs) to subscribed receivers in seconds, using a standardised wire format
defined by the OpenID Foundation. No per-vendor webhooks, no bespoke polling endpoints, no Kafka topic per integration.
